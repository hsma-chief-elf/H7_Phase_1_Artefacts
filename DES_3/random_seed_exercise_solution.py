...
class HSMA:
    def __init__(self, id, rng):
        self.id = id
        self.py_confidence = rng.choice(["Weak", "Normal", "Strong"])
        print (self.py_confidence)
...
class Model:
    ...
    def generator_hsma_arrivals(self):
        while True:
            self.hsma_counter += 1
            associate = HSMA(self.hsma_counter, self.hsma_confidence_rng)
